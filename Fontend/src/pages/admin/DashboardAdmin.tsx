...
const DashboardAdmin = () => {
  ...
  const monthlyData = [
    { month: 'มกราคม', income: 300, bookings: 50, payments: 30 },
    { month: 'กุมภาพันธ์', income: 500, bookings: 70, payments: 40 },
    { month: 'มีนาคม', income: 800, bookings: 90, payments: 60 },
    { month: 'เมษายน', income: 800, bookings: 90, payments: 60 },
    { month: 'พฤษภาคม', income: 800, bookings: 90, payments: 60 },
    { month: 'มิถุนายน', income: 800, bookings: 90, payments: 60 },
    { month: 'กรกฎาคม', income: 800, bookings: 90, payments: 60 },
    { month: 'สิงหาคม', income: 800, bookings: 90, payments: 60 },
    { month: 'กันยายน', income: 800, bookings: 90, payments: 60 },
    { month: 'ตุลาคม', income: 800, bookings: 90, payments: 60 },
    { month: 'พฤศจิกายน', income: 800, bookings: 90, payments: 60 },
    { month: 'ธันวาคม', income: 800, bookings: 90, payments: 60 },
  ];
    ...
    <Box sx={{ marginTop: 9.5, marginLeft: 30 }}>
      ...
      <Stack spacing={2} direction="row" sx={{ alignItems: "center" }}>
          ...
          <Input slotProps={{
            ...
            <h4>ตรวจสอบเป็นเวลา {selectDateD !== 0 && selectDateD + " วัน"} {selectDateM !== 0 && selectDateM + " เดือน"} {selectDateY !== 0 && selectDateY + " ปี"}</h4>
        </Stack>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center', marginTop: 2 }}>
        <Button
          variant={chartType === 'week' ? 'contained' : 'outlined'}
          onClick={() => handleChartTypeChange('week')}
        >
          รายสัปดาห์
        </Button>
        <Button
          variant={chartType === 'month' ? 'contained' : 'outlined'}
      ...
      <Card sx={{ marginTop: 3, padding: 2, width: 1213 }} variant="outlined" >
        <LineChart width={1230} height={350} data={monthlyData} style={{ marginLeft: -28 }}>
          <CartesianGrid strokeDasharray="3 3" />
          <XAxis dataKey="month" />
          <YAxis />
          <Tooltip />
          <Legend />
          <Line type="monotone" dataKey="income" stroke="#8884d8" name="รายได้" />
          <Line type="monotone" dataKey="bookings" stroke="#82ca9d" name="จำนวนการจอง" />
          <Line type="monotone" dataKey="payments" stroke="#ffc658" name="จำนวนที่ชำระเงิน" />
        </LineChart>
      </Card>
    </Box>
  );
